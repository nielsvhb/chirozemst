		...
		<div class="col s8 offset-s2 event-create">
			...
			<div class="row">
				<div class="col s8 offset-s2 m4 offset-m4">
					<a class="waves-effect waves-light btn" onclick="document.getElementById('form-id').submit()"><i class="material-icons left">done</i>maak evenement</a>
				</div>
			</div>
		</div>
	</div>
		...
		<ul class="collection">
			@foreach( $eventsByDate as $event)
				<li class="collection-item">
					<span class="title hover_class">{{ $event->title }}<a href="{{ route('event.delete', $event->id) }}"><i class="material-icons icon-delete">delete</i></a></span>
